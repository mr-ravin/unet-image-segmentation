...
def train(loader,model,optimizer,loss_fn,scaler):
  ...
  for batch_idx, (input_data,targets) in enumerate(loop):
    ...
    targets = targets.float().unsqueeze(1).to(device=DEVICE) # shift to GPU if enabled.
...
def run():  # max_pixel_value=255.0 set to get value between 0.0 and 1.0 for pixel values 
  ...
  train_loader, validation_loader = utils.get_loaders( TRAIN_IMAGEDIR_PATH,TRAIN_MASKDIR_PATH,VAL_IMAGEDIR_PATH,VAL_MASKDIR_PATH,
